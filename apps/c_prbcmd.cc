...
public: // OUTPUT
  ...
  void flush(){ untested(); }
private:
//  static std::map<std::string, OUTPUT_TEE>;
//
//  // hmm, who owns the tees?
...
protected:
  OUTPUT_CMD() : OUTPUT() /*SIM()*/ {unreachable();}
public: // maybe that should really go to a different class...
...
public: // hmm
  void do_head(double x, double y, const std::string& s, PROBELIST const*p){
    return do_head(x, y, s, *p);
  }
  void do_outdata(double x, PROBELIST const*p){
    return do_outdata(x, *p);
  }
public:
...
public:
  CMD_PLOT() : OUTPUT_CMD("alarm", ofPRINT) {}
public:
...
/* dB for fourier (hack)
 */
static double fodb(COMPLEX Value)
{ untested();
...
public:
  ...
  { untested();
   ...
   //  }else
    ...
    for (PROBELIST::const_iterator
	 p=pr.begin(); p!=pr.end(); ++p) { untested();
      switch(p->_type){ untested();
	case pREAL:
	  OUTPUT::_out.form(format, ' ', p->label().c_str());
	  break;
	case pCOMPLEX:
	  OUTPUT::_out.form(format, ' ', p->label().c_str());
	  OUTPUT::_out.form(format, ' ', p->label("db").c_str());
	  OUTPUT::_out.form(format, ' ', p->label("p").c_str());
	  break;
	default: unreachable();
      }
    }
  ...
  { untested();
    ...
    // }else
    if (IO::plotout.any() && plt.has_probes()) { untested();
