...
class CMD_FUNC: public CMD {
  private:
    std::string _func_name,_expression;
  ...
  public: 
    ...
    _expression(expression_str){
     ...
     void do_it(CS& cmd,CARD_LIST* Scope){
       cmd >> "(" >> _arg1 >> "," >> _arg2 >> "," >> _arg3 >>")";
       IO::mstdout << trig_function<double>(_expression,_arg1,_arg2,_arg3) << "\n";
     }
};
...
public:
  void do_it(CS& cmd, CARD_LIST*) {
    ...
    cmd >> func_name;
    if(func_name==""){
      throw Exception("Usage:def [function_name]");
    }else{
    }
    IO::mstdout << ">";
